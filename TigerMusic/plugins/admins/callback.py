...
from TigerMusic import YouTube, app
from TigerMusic.core.call import Tiger
from TigerMusic.misc import SUDOERS, db
from TigerMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from TigerMusic.utils.decorators.language import languageCB
from TigerMusic.utils.formatters import seconds_to_min
from TigerMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from TigerMusic.utils.stream.autoclear import auto_clean
from TigerMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Tiger.pause_stream(chat_id)
        ...
        await Tiger.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Tiger.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Tiger.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Tiger.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Tiger.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Tiger.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
